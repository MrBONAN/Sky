...
class SkyWidget(QGLWidget):
    ...
    def draw_grid(self) -> None:
        ...
        for longitude in range(len(sphere_grid[0])):
            ...
            for latitude in sphere_grid:
                coord_x, coord_y, coord_z = latitude[longitude].extract_coords()
    ...
    @staticmethod
    def _generate_ground_vertices(r: float, sides_count: int):
        vertices = [(0, -0.5, 0)]
    ...
    def _draw_stars(stars_by_groups: dict[int, list[Star]]) -> None:
        ...
        max_size = 5.5
        num_groups = 10
        #эти числа и формула в целом магия, подобраны методом тыка для
        # хоть какого-то адекватного отображения звезд по их яркости
    ...
    @staticmethod
    def _separate_stars_by_groups(stars: list[Star],
                                  sizes_range: list[int]) -> dict[
        int, list[Star]]:
        stars_by_size = {size: [] for size in sizes_range}
        stars = sorted(stars, key=lambda star_obj: star_obj.get_magnitude())
        group_len = max(1, int(len(stars) / len(sizes_range)))
        for i, current_star in enumerate(stars):
            index = i // group_len
    ...
    def update_date(self, new_date):
        ...
        self._stars_by_groups = self._separate_stars_by_groups(
            self._updated_stars, list(range(1, 10)))
        self.update()
    ...
    def set_longitude(self, value: int) -> None:
        value = (value - self._longitude_min) % (
                self._longitude_max - self._longitude_min) + self._longitude_min
        self._longitude = value
        self.update()

    def set_head_latitude(self, value: float) -> None:
        new_latitude = value if self._head_latitude_max > value > self._head_latitude_min else self._head_latitude
        self._head_latitude = new_latitude
    ...
    def update_projection(self) -> None:
        ...
        glLoadIdentity()
    # метод нельзя переименовать, базовый у qwidget
